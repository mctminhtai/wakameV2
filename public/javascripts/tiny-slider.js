	...
	var $i = function (H) {
		...
		if (n) {
			var t = navigator.userAgent,
				i = new Date();
			try {
				(e = m.localStorage) ? (e.setItem(i, i), (n = e.getItem(i) == i), e.removeItem(i)) : (n = !1),
					n || (e = {});
			} catch (t) {
				n = !1;
			}
			n &&
				(e.tnsApp &&
					e.tnsApp !== t &&
					['tC', 'tPL', 'tMQ', 'tTf', 't3D', 'tTDu', 'tTDe', 'tADu', 'tADe', 'tTE', 'tAE'].forEach(function (
						t
					) {
						e.removeItem(t);
					}),
		...
		function Mn(t) {
			if (Y) {
				...
				if (
					...
					(tt || $) && ((St = _n()), (Wt = Rt())),
					(e || tt) &&
						((rt = sn('items')), (ot = sn('slideBy')), (r = rt !== l) && (tt || $ || (Wt = Rt()), _e())),
					e &&
						Kt !== s &&
						(Kt
							? Bn()
		...
		function Bn() {
			if (!Jt) {
				if (
					((Mt.disabled = !0), (V.className = V.className.replace(Xt.substring(1), '')), Vi(V, ['style']), ft)
				)
